...
function validateMaxResults() {
  ...
  if (inputVal.match(/^\d+$/) && Number.isInteger(parseInt(inputVal))) {
    ...
    if (num < Number.MAX_SAFE_INTEGER) {
      ...
      markStatus(inputVal + " est trop grand.", 2000);
      document.getElementById('maxResults').style.backgroundColor = ERROR_COLOR;
    }
  } else {
    markStatus("'" + inputVal + "' n'est pas un nombre entier. Veuillez essayer une autre valeur.", 2000);
    document.getElementById('maxResults').style.backgroundColor = ERROR_COLOR;
  }
  return false;
}
...
function saveOptions() {
  ...
  if (maxResults) {
    ...
    chrome.storage.local.set(options, function() {
      markStatus('Nouveaux param\350tres enregistr\351s');
    });
  }
}
...
function restoreDefaults() {
  chrome.storage.local.clear(function() {
    markStatus('Valeurs par d\351faut restaur\351es');
  });
  loadOptions();
}
/*** FUNCTIONS ***/
